...
void capScreenForm::capStart()
{
	...
	while (true)
	{
		...
		//qDebug() << "r-" << wRect->right << "l-" << wRect->left << "b-" << wRect->bottom << "t-" << wRect->top << endl;
		int width = wRect->right - wRect->left; //width = width * 2 / 3;
		int height = wRect->bottom - wRect->top; //height = height * 2 / 3;
